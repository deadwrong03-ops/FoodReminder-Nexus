...
Write-Host "FoodReminder-Nexus itemID mapper"
Write-Host "Source: $sourcePath"
Write-Host ""

function Download-GW2ItemCatalog {
    ...
    for ($i = 0; $i -lt $allIds.Count; $i += $batchSize) {
        ...
        Write-Host ("Downloaded batch {0}/{1} ({2} items total)" -f $batchNumber, $totalBatches, $allItems.Count)
    }
...
if (Test-Path $cachePath) {
    try {
        ...
        if ($cached -and @($cached).Count -gt 1000) {
            $items = @($cached)
            $needDownload = $false
...
if ($needDownload) {
    $items = Download-GW2ItemCatalog
    $items | ConvertTo-Json -Depth 20 | Set-Content $cachePath -Encoding UTF8
    Write-Host ("Catalog cached: {0} ({1} items)" -f $cachePath, $items.Count)
}

# Exact English-name index for Consumable items.
$nameIndex = @{}
...
# Matches:
# { 17825, { "Power", "Name", "Effects" } }
# { 17825, { "Power", "Name", "Effects", 41569 } }
$cppString = '"(?:\\.|[^"\\])*"'
